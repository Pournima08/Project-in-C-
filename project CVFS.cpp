...
int ReadFile(int fd,char *arr,int isize)
{
	...
	if(UFDTArr[fd].ptrtfiletable->mode!=READ && UFDTArr[fd].ptrfiletable->mode!=READ+WRITE)
	...
	return -2;
	 if(UFDTArr[fd].ptrfiletable->readoffest==UFDTArr[fd].ptrfiletable->ptrinode->FileActualSize)
	 return -3;
...
int LseekFile(int fd,int size,int from)
{
	...
	if((UFDTArr[fd].ptrfiletable->mode==READ)||(UFDTArr[fd].ptrfiletable->mode==READ+WRITE))
	{
		if(from==CURRENT)
		{
		 ...
		 if(((UFDTArr[fd].ptrfiletable->readoffset)=(UFDTArr[fd].ptrfiletable->readoffset)+size;
		 }
	else if(from==START)
	{
...
printf("\n____Statistical Information about file________\n");
printf("File name:%s\n",temp->FileName);
...
count=sscanf(str,"%s 5s %s",command[0],command[1],command[2],command[3]);
...
{
	ifstrcmp(command[0],"stat")==0)
	{
		ret=stat_file(command[1]);
		if(ret==-1)
		printf("ERROR:Incorrect Prameters\n");
		if(ret==-2)
	...
	if(ret==0)
	{
		printf("ERROR:Incoreect parameter\n");
		continue;
	}
	...
	if(ret==-2)
		printf("ERROR:There is no sufficient memoery to write\n");
	if(ret==-3)
